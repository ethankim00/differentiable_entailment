...
def save_predictions(path: str, wrapper, results: Dict):
    ...
    if wrapper.task_helper and wrapper.task_helper.output:
        predictions_with_idx = wrapper.task_helper.output  # use model task helper
    else:
        inv_label_map = {idx: label for label, idx in wrapper.label_map.items()}
        for idx, prediction_idx in zip(
            results["indices"], results["predictions"]
        ):  # predictions are already stored in results dict
...
def write_results(path: str, dev_results: Dict, eval_results: Dict):
    with open(path, "w") as fh:
        ...
        for metric in results.keys():
            ...
            all_stdev = statistics.stdev(eval_results) if len(eval_results) > 1 else 0
        ...
        for metric in dev_results.keys():
            eval_results = [
                result
                for pattern_results in dev_results[metric].values()
                for result in pattern_results
            ]
            all_mean = statistics.mean(eval_results)
            all_stdev = statistics.stdev(eval_results) if len(eval_results) > 1 else 0
...
def trim_input_ids(
    ...
    for input_id in input_ids_without_pad:
        if input_id == mask_token_id:
            if mask_count >= num_masks:  # skip excessive mask tokens
                continue
            mask_count += 1
